...
var Node = class _Node {
  ...
  insert(tokens2, index, paramMap, context2, pathErrorCheckOnly) {
    if (tokens2.length === 0) {
...
function initializeTokens(env2) {
  tokens = [];
  if (env2.QWEN_TOKEN) {
    tokens.push({ token: env2.QWEN_TOKEN, status: "OK", id: "env_token_1" });
  }
  if (env2.QWEN_TOKENS) {
    const multiTokens = env2.QWEN_TOKENS.split(",").map((t) => t.trim()).filter((t) => t);
...
__name(initializeTokens, "initializeTokens");
...
__name(sendQwenRequest, "sendQwenRequest");
...
function getApiKeys(env2) {
  if (env2.API_KEYS) {
    return env2.API_KEYS.split(",").map((k) => k.trim()).filter((k) => k);
  }
  return [];
}
...
app.get("/api/models", async (c) => {
  try {
    ...
    console.error("Error fetching models:", error3);
...
app.get("/api/status", async (c) => {
  try {
    ...
    console.error("Error getting status:", error3);
...
app.post("/api/chat", async (c) => {
  try {
    ...
    return c.json({ error: "Internal server error", details: error3.message }, 500);
  }
});
app.post("/api/chat/completions", async (c) => {
  try {
    ...
    console.error("Error in OpenAI completions endpoint:", error3);
    return c.json({
      error: {
        message: error3.message || "Internal server error",
        type: "internal_error"
      }
    }, 500);
  }
...
app.get("/api/tasks/status/:taskId", async (c) => {
  try {
    ...
    console.error("Error polling task status:", error3);
...
var worker_default = {
  async fetch(request, env2, ctx) {
    if (tokens.length === 0) {
      const count3 = initializeTokens(env2);
      console.log(`Initialized ${count3} token(s) from environment`);
      if (count3 === 0) {
...
// .wrangler/tmp/bundle-MNyhKY/middleware-insertion-facade.js
...
// .wrangler/tmp/bundle-MNyhKY/middleware-loader.entry.ts
